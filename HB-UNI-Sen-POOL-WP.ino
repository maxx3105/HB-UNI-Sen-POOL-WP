...
#define NTC_SENSE_PIN_3      A2 //PA2
                                //PA3
                                //PA4
#define PRESSURE_PIN         A5 //PA5
#define CONFIG_BUTTON_PIN    A6 //PA6
#define ORPH_SIGNAL_PIN      A7 //PA7

#define LED_PIN              0  //PB0
#define SC_1_PIN             1  //PB1
#define SC_2_PIN             2  //PB2
#define SC_3_PIN             3  //PB3
...
DEFREGISTER(UReg1, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b)
class UList1 : public RegList1<UReg1> {
  public:
    ...
    bool TemperatureOffsetIndex3 (uint8_t value) const { return this->writeRegister(0x07, value & 0xff); }
    uint8_t TemperatureOffsetIndex3 () const { return this->readRegister(0x07, 0); }
    ...
    bool FlowRateQFactor (uint16_t value) const { return this->writeRegister(0x08, (value >> 8) & 0xff) && this->writeRegister(0x09, value & 0xff); }
    uint16_t FlowRateQFactor () const { return (this->readRegister(0x08, 0) << 8) + this->readRegister(0x09, 0); }

    bool ToggleWaitTime (uint16_t value) const { return this->writeRegister(0x0a, (value >> 8) & 0xff) && this->writeRegister(0x0b, value & 0xff); }
    uint16_t ToggleWaitTime () const { return (this->readRegister(0x0a, 0) << 8) + this->readRegister(0x0b, 0); }

    void defaults () {
      clear();
      TemperatureOffsetIndex1(7);
      TemperatureOffsetIndex2(7);
      TemperatureOffsetIndex3(7);
      OrpOffset(0);
      FlowRateQFactor(10);
      ToggleWaitTime(200);
    }
...
public:
  ...
  void showMeasureValues(int16_t t1, int16_t t2, int16_t t3, uint16_t ph, int16_t orp, uint16_t pressure, uint8_t flow) {
    lcd.clear();
    lcd.setCursor(0,0);
    lcd.print("  T1  |  PH  |  ORP ");
...
class MeasureEventMsg : public Message {
  public:
    void init(uint8_t msgcnt, int16_t temp, int16_t temp2, int16_t temp3, uint16_t ph, int16_t orp, uint16_t pressure, uint8_t flow) {
      Message::init(0x19, msgcnt, 0x53, BIDI | WKMEUP, 0x41, (temp >> 8) & 0xff);
      pload[0] = temp & 0xff;
      pload[1] = (ph >> 8) & 0xff;
      pload[2] =  ph & 0xff;
      pload[3] = (orp >> 8) & 0xff;
...
private:
    ...
    Ntc<NTC_SENSE_PIN_3,NTC_R0,NTC_B,0 ,NTC_T0,NTC_OVERSAMPLING> ntc3;
    PCF8583           pcf8583;
    ...
    uint32_t          temperature3_cumulated;
    uint8_t           flowrate;
  ...
  public:
    ...
    int16_t readTemperature3() {
      ntc3.measure();
      int16_t t = ntc3.temperature() + (-14+2*this->getList1().TemperatureOffsetIndex3());
      DPRINT(F("3. Temperatur     : "));DDECLN(t);
      return t;
    }
    ...
    void run() {
      ...
      lcd.showMeasureValues(currentTemperature1, currentTemperature2, currentTemperature3, ph, orp, pressure, flowrate);
      ...
      temperature3_cumulated += currentTemperature3;
      ...
      if (measureCount >= device().getList0().Sendeintervall()) {
        msg.init(device().nextcount(),
                 ...
                 temperature3_cumulated / measureCount,
    ...
    void configChanged() {
      ...
      DPRINT(F("*3.Temperature Offset : "));DDECLN(this->getList1().TemperatureOffsetIndex3());
...
class DummyChannel : public Channel<Hal, UList1, EmptyList, DefList4, 2, UList0> {
public:
  typedef Channel<Hal,EmptyList,EmptyList,DefList4,2,UList0> BaseChannel;
  uint8_t status () const { return 0; }
  uint8_t flags ()  const { return 0; }
};

class UType : public ChannelDevice<Hal, VirtBaseChannel<Hal, UList0>, 10, UList0> {
  public:
    VirtChannel<Hal, SWChannel, UList0>         channel7, channel8, channel9, channel10;
    VirtChannel<Hal, SCChannel, UList0>         channel4, channel5, channel6;
    VirtChannel<Hal, MeasureChannel, UList0>    channel1;
    VirtChannel<Hal, DummyChannel, UList0>      channel2, channel3;
  public:
    typedef ChannelDevice<Hal, VirtBaseChannel<Hal, UList0>, 10, UList0> DeviceType;
    ...
    UType (const DeviceInfo& info, uint16_t addr) : DeviceType(info, addr) {
      ...
      DeviceType::registerChannel(channel9, 9);
      DeviceType::registerChannel(channel10, 10);
    }
    virtual ~UType () {}
...
InternalButton<UType> swbtn1(sdev,7);
InternalButton<UType> swbtn2(sdev,8);
InternalButton<UType> swbtn3(sdev,9);
InternalButton<UType> swbtn4(sdev,10);

void initPeerings (bool first) {
  if ( first == true ) {
    HMID devid;
    sdev.getDeviceID(devid);
    sdev.swChannel1().peer(Peer(devid, 7));
    sdev.swChannel2().peer(Peer(devid, 8));
    sdev.swChannel3().peer(Peer(devid, 9));
    sdev.swChannel4().peer(Peer(devid, 10));
  }
}
